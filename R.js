document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('AutoTable version:', window.jspdf?.AutoTable?.version);
    ...
    async function loadDB() {
        try {
            ...
            await initializeDefaultDB(); // Создаем стандартную БД при ошибке
        }
    }
    ...
    function setupReports() {
        ...
        if (generateReportBtn) {
            ...
            console.warn('Кнопка generate-report не найдена');
        }

        if (printReportBtn) {
            printReportBtn.addEventListener('click', printReport);
        } else {
            console.warn('Кнопка print-report не найдена');
        }

        if (exportPdfBtn) {
            exportPdfBtn.addEventListener('click', exportToPDF);
        } else {
            console.warn('Кнопка export-pdf не найдена');
        }
    }
    ...
    function exportToPDF() {
        try {
            ...
            // Устанавливаем стандартный шрифт, поддерживающий кириллицу
            doc.setFont('courier'); // Используем моноширинный шрифт
            ...
            // Заголовок отчета (без использования bold, так как он может не поддерживаться)
            doc.setFontSize(16);
            doc.text(getReportTitle(reportType), margin, currentY);
            currentY += 10;
            ...
            doc.text(`Сформировано: ${userName}, ${dateStr}`, margin, currentY);
            currentY += 15;

            // Получаем данные для таблицы
            const { columns, data } = prepareReportData(reportType);
            
            // Рассчитываем ширину колонок
            const colCount = columns.length;
            const colWidth = (pageWidth - 2 * margin) / colCount;
            
            // Рисуем заголовки таблицы (без заливки цветом)
            doc.setFontSize(12);
            doc.setTextColor(0, 0, 0); // Черный текст
            ...
            columns.forEach((col, index) => {
                doc.text(
                    ...
                    currentY + 5,
                    { align: 'center', maxWidth: colWidth - 4 }
                );
            });
            ...
            // Добавляем прогноз для отчета по остаткам
            if (reportType === 'stock') {
                const modelingResults = modelStockConsumption();
                currentY += 10;
                doc.setFontSize(10);
                doc.text(`Прогноз исчерпания запасов: ${modelingResults.daysToStockout} дней`, margin, currentY);
                currentY += 5;
                doc.text(`Товары с низким запасом: ${modelingResults.lowStockItems.join(', ')}`, margin, currentY);
            }

            // Сохраняем PDF
            doc.save(`Отчет_${getReportTitle(reportType)}_${dateStr.replace(/\./g, '-')}.pdf`);
        } catch (error) {
    ...
    function prepareReportData(reportType) {
        ...
        switch(reportType) {
                ...
                data = db.products
                    ...
                    .map(p => {
                        ...
                        return [
                            ...
                            p.quantity, 
                            p.expiry, 
                            diffDays > 0 ? diffDays.toString() : 'Просрочено'
                        ];
                    });
                break;
            ...
            default:
                columns = ['Артикул', 'Наименование', 'Количество'];
                data = db.products.map(p => [p.article, p.name, p.quantity]);
        }
